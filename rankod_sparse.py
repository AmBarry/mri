...
for subject in os.listdir(root):
    ...
    if os.path.exists(brain_path):
        ...
        mri_data.append(brain)
        mri_label.append(subject)
        i = i + 1
mri_data = np.squeeze(np.array(mri_data))
mri_data = mri_data[:, np.all(mri_data != 0, axis=0)]

with open("mri_labels.csv", 'w', newline='') as label_csv_f:
    wr = csv.writer(label_csv_f, quoting=csv.QUOTE_ALL)
    wr.writerow(mri_label)

outlier_labels = []
with open("outlier_labels.txt") as outlier_f:
    for line in outlier_f:
        outlier_labels.append("sub-00" + line.rstrip())


pca = KernelPCA(n_components = 1053, n_jobs = 1, kernel = "linear")
mri_pca = pca.fit_transform(mri_data)
mri_pca = mri_data[:, 300000 : 350000]

outlier_scores = lesinn(mri_pca)
np.savetxt("sp_scores.csv", outlier_scores)


# # test_single_embedding(mri_pca, labels, outlier_scores, "REPEN.csv")
houtliers = np.genfromtxt('high_dim_outlier_data.csv',delimiter=',', skip_header = 1, dtype = None)
houtliers_dict = {}
for sub in range(len(houtliers[:,1])):
    houtliers_dict["sub-00" + str(houtliers[sub, 1])] = houtliers[sub, 4]

labels = []
for l in mri_label:
    if l in houtliers_dict:
        labels.append(houtliers_dict[l])
    else:
        labels.append(0)
...
import scipy.sparse
mri_pca_sparse = scipy.sparse.csc_matrix(mri_pca)
X_dk_sparse = scipy.sparse.csc_matrix(X_dk)

embedding_size = 20
single_embedding_with_priorknowledge(mri_pca_sparse, labels, embedding_size, X_dk_sparse, outlier_scores, "PRIOR")
